...
const PROJECTS = [
  ...
  },
  {
    ...
    tasks: [
      ...
      { name: 'Logo设计', status: 'active' },
      { name: '应用场景设计', status: 'pending' }
    ]
  }
];

export default function ProjectList({ searchTerm, filterStatus }: ProjectListProps) {
              ...
              {project.tasks.map((task, index) => (
                <div key={index} className="flex items-center justify-between text-sm">
                  ...
                  <span className={`text-xs ${
                    ...
                    task.status === 'active' ? 'text-yellow-500' : 'text-gray-400'
                  }`}>
                    {task.status === 'completed' ? '已完成' :
                     task.status === 'active' ? '进行中' : '待开始'}
                  </span>
                </div>
              ))}
            </div>
